...
void	ft_raycasting(t_cub *cub, int *x)
{
	...
	{
		cub->ray.step_x = -1;
		cub->ray.x_dist = (cub->player.x_coord - cub->ray.map_x) * cub->ray.delta_dist_x;
	}
	else
	{
		cub->ray.step_x = 1;
		cub->ray.x_dist = (cub->ray.map_x + 1.0 - cub->player.x_coord) * cub->ray.delta_dist_x;
	}
	if (cub->ray.dir_y < 0)
	{
		cub->ray.step_y = -1;
		cub->ray.y_dist = (cub->player.y_coord - cub->ray.map_y) * cub->ray.delta_dist_y;
	}
	else
	{
		cub->ray.step_y = 1;
		cub->ray.y_dist = (cub->ray.map_y + 1.0 - cub->player.y_coord) * cub->ray.delta_dist_y;
	}
	while (!hit)
	{
	...
	if (!cub->ray.side)
		cub->ray.wall_dist = (cub->ray.map_x - cub->player.x_coord + (1 - cub->ray.step_x) / 2) / cub->ray.dir_x;
	else
		cub->ray.wall_dist = (cub->ray.map_y - cub->player.y_coord + (1 - cub->ray.step_y) / 2) / cub->ray.dir_y;
